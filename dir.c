...
struct page *rkfs_get_page(struct inode *vfs_pinode, unsigned long n)
{
    ...
    page = read_cache_page(mapping,n,(filler_t*)mapping->a_ops->readpage,NULL);
    if( IS_ERR(page) ) {
...
int rkfs_readdir(struct file *filp, void *dirent, filldir_t filldir)
{
    ...
    rkfs_debug("Inode: %ld, sz: %ld, pgs= %ld, pos= %ld, n = %ld, offt= %d\n",
    ...
    while( n < npages ) {
        ...
        if( IS_ERR(page) ) {
            rkfs_debug("Got page error while reading page no: %ld\n",n);
            continue;
        }
        ...
        while( p_addr <= pe_addr ) {
            ...
            if( de->de_inode ) {
                ...
                if( over ) {
                    rkfs_put_page(page);
                    rkfs_debug("filldir returned over=: %d\n",over);
                    goto done;
                }
            }
            ...
            if( de->de_name_len ) {
                ...
                rkfs_debug("namelen = 0, so no next entry...\n");
                break;
            }
        }
    ...
    *res_page = NULL;
    ...
    for( n=0; n<npages; n++ ) {
        ...
        while( p_addr <= pe_addr ) {
            ...
            if( de->de_inode && !de->de_name_len ) {
                rkfs_bug("Invalid dir entry found in page %ld\n",n);
                rkfs_put_page(page);
                FAILED;
                goto not_found;
            }
    ...
    for( n=0; n<=npages; n++ ) {
        ...
        while( p_addr <= pe_addr ) {
            ...
            if( de->de_inode ) {
                ...
                if( !de->de_name_len ) {
                    rec_len = reclen;
                    rkfs_debug("Got new free dir-entry slot\n");
                    goto got_it;
                }

                rec_len = RKFS_DIR_ENTRY_LEN(de->de_name_len);
                if( rec_len == reclen ) {
                    rkfs_debug("Got exact free dir-entry slot of size: %d\n",
                                rec_len);
                    goto got_it;
                }

                if( (rec_len > reclen) &&
                    (rec_len - reclen) >= RKFS_DIR_ENTRY_LEN(1) ) {
                    rkfs_debug("Got free bigger dir-entry slot of size: %d\n",
                                rec_len);
                    goto got_it;
                }

                rkfs_debug("Skipping empty dir-entry slot of size: %d\n",
                            rec_len);
            }
...
int rkfs_make_empty(struct inode *inode, struct inode *parent)
{
    ...
    if( err ) {
        rkfs_debug("Can't make empty directory (fail to prepare write)\n");
        goto fail;
    }
    ...
    if( err ) {
        rkfs_debug("Can't make empty directory (fail to commit chunk)\n");
        goto fail;
    }
...
int rkfs_empty_dir(struct inode *inode)
{
    ...
    for( n=0; n<npages; n++ ) {
        ...
        while( p_addr <= pe_addr ) {
            ...
            if( de->de_inode ) {
                if( !de->de_name_len ) {
                    rkfs_bug("Invalid dir entry found at page %ld\n",n);
                    goto not_empty;
                }
                ...
                if( (de->de_name_len == 2) && (de->de_name[0] != '.') &&
                    (de->de_name[1] != '.') )
                    goto not_empty;
